...
void Adafruit_BNO055::setSensorOffsets(const uint8_t* calibData) {
    ...
    /* Note: Configuration will take place only when user writes to the last
       uint8_t of each config data pair (ex. ACCEL_OFFSET_Z_MSB_ADDR, etc.).
       Therefore the last uint8_t must be written whenever the user wants to
...
imu::Vector<3> Adafruit_BNO055::getEulerFromQ() {
    ...
    imu::Vector<3> euler(yaw, roll, pitch);
    return euler;
}